...
scan_kernel(
  ...
  volatile uint32_t *s_dynid_counter = (volatile uint32_t*) smem_ext;

  volatile ElTp *s_chunks           = (volatile ElTp*) smem_ext;
  volatile ElTp *s_blockscan_buf    = s_chunks + B * Q;
  volatile ElTp *s_block_exc_prefix = s_blockscan_buf;

  volatile ElTp *s_chunk = s_chunks + threadIdx.x * Q;
   ...
   *    results in smem.
   */
  copy_gmem_to_smem<OP, B, Q>(g_in, s_chunks, tblock_offset, N);
  __syncthreads();

  ElTp acc = OP::ne();
  #pragma unroll
  for (int i = 0; i < Q; i++)
    s_chunks[threadIdx.x * Q + i]
      = acc
      = OP::apply(acc, s_chunks[threadIdx.x * Q + i]);
  ...
  for (int i = 0; i < Q; i++)
    s_chunks[threadIdx.x * Q + i]
      = OP::apply(thread_exc_prefix, s_chunks[threadIdx.x * Q + i]);
